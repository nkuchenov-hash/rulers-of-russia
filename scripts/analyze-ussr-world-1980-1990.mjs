...
const index=JSON.parse(fs.readFileSync('public/data/territory/world-history/index.json','utf8'));
const available=(index.snapshots??[]).map(x=>x.year).sort((a,b)=>a-b);
const nearest=target=>available.reduce((best,y)=>Math.abs(y-target)<Math.abs(best-target)?y:best,available[0]);
const years=[...new Set([nearest(1980),nearest(1990)])];
const report={schema_version:2,availableYears:available,requestedTargets:[1980,1990],selectedYears:years,years:[]};
