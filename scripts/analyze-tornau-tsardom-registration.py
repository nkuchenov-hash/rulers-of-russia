...
TARGET = {
    "id": "1598-1682",
    "url": "https://upload.wikimedia.org/wikipedia/commons/d/df/Historical_map_of_Russian%2C_1598-1682.gif",
    # SHA-1 published by Wikimedia Commons structured file metadata.
...
def align(reference: np.ndarray, target: np.ndarray) -> dict:
    ...
    reusable = (
        ...
        and inlier_count >= 50
        and ratio >= 0.30
        and fstats["median"] is not None
        and fstats["median"] <= 3.0
        and fstats["p95"] is not None
        and fstats["p95"] <= 8.0
        and rstats["median"] is not None
        and rstats["median"] <= 0.1
    ...
    for gcp_id, x, y, lon, lat in REFERENCE_GCPS:
        ...
        transferred.append({
            "id": gcp_id,
            "sourceReferencePixel": [x, y],
            "targetCandidatePixel": [round(float(px[0]), 2), round(float(px[1]), 2)],
            "lonLat": [lon, lat],
            "basis": "candidate transferred from pinned 862 Tornau GCP by SIFT/RANSAC raster homography",
        })
...
def main() -> None:
    ...
    reference_bytes = request_bytes(REFERENCE["url"])
    target_bytes = request_bytes(TARGET["url"])
    ...
    reference = decode_gray(reference_bytes, REFERENCE)
    target = decode_gray(target_bytes, TARGET)
    registration = align(reference, target)
    ...
    report = {
        ...
        "reference": {
            **REFERENCE,
            "observedBytes": len(reference_bytes),
            "observedSha256": reference_sha256,
        },
        "target": {
            **TARGET,
            "observedBytes": len(target_bytes),
            "observedSha1": target_sha1,
            "rightsStatus": "public-domain",
            "rightsBasis": "Wikimedia Commons file page / Public Domain Mark; published 1910",
        },
        "registration": registration,
        "promotionGate": {
            "automaticPromotionAllowed": False,
            "requirements": [
                "registration candidateReusable must be true",
                "transferred anchors must be independently checked against recognizable target-sheet geography",
                "1598 legend-specific spatial components must be reviewed against those controls",
                "derived boundary must pass History Core spatial and strict completion audits",
            ],
        },
    }

    path = OUT / "registration-report.json"
