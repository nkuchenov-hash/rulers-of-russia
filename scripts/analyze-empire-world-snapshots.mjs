...
const controls1815=[
['moscow',[37.6173,55.7558],'inside'],['st-petersburg',[30.3351,59.9343],'inside'],['helsinki',[24.9384,60.1699],'inside'],['warsaw',[21.0122,52.2297],'inside'],['kyiv',[30.5234,50.4501],'inside'],['tbilisi',[44.793,41.7151],'inside'],['yerevan',[44.5152,40.1872],'outside'],['baku',[49.8671,40.4093],'inside'],['irkutsk',[104.2807,52.2864],'inside'],['yakutsk',[129.7326,62.0355],'inside'],['petropavlovsk-kamchatsky',[158.65,53.037],'inside'],['sitka',[-135.33,57.053],'inside'],
['berlin',[13.405,52.52],'outside'],['vienna',[16.3738,48.2082],'outside'],['istanbul',[28.9784,41.0082],'outside'],['tehran',[51.389,35.6892],'outside'],['beijing',[116.4074,39.9042],'outside'],['stockholm',[18.0686,59.3293],'outside'],['bucharest',[26.1025,44.4268],'outside']
];
const controls1914=[
...
for(const year of years){
 ...
 const yearControls=year===1815?controls1815:year===1914?controls1914:null;
 const aggregateControls=yearControls?yearControls.map(([id,p,e])=>{const a=unionContains(p,russianSubject.map(x=>x.f))?'inside':'outside';return {id,expected:e,actual:a,pass:a===e}}):null;
 const failedPointContainers=yearControls?yearControls.filter(([,p,e])=>e==='inside').map(([id,p])=>({id,containers:(d.features??[]).map((f,i)=>({i,f,p:f.properties??{}})).filter(x=>contains(p,x.f.geometry)).map(x=>({index:x.i,properties:x.p,bbox:bbox(x.f.geometry)}))})):null;
