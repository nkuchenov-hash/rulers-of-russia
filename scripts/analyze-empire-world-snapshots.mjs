...
for(const year of years){
 ...
 report.years.push({year,candidates:candidates.map(x=>({index:x.i,properties:x.p,geometry_type:x.f.geometry?.type,bbox:bbox(x.f.geometry),controls:year===1914?controls1914.map(([id,p,e])=>{const a=contains(p,x.f.geometry)?'inside':'outside';return {id,expected:e,actual:a,pass:a===e}}):null}))});
}
