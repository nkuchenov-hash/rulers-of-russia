...
function startInertia(controls, state) {
  ...
  const frame = (now) => {
    ...
    const distanceFactor = clamp((state.spherical.radius - controls.minDistance) / 1.45, 0, 1);
    const decay = Math.pow(0.984 + distanceFactor * 0.008, frameScale);
    ...
    if (Math.abs(state.velocityTheta) + Math.abs(state.velocityPhi) < 0.000007) {
      state.raf = null;
      return;
    }
...
function connectHeavy() {
  ...
  state.onPointerMove = (event) => {
    ...
    const zoomFactor = clamp((radius - this.minDistance) / 1.45, 0, 1);
    const speedFactor = clamp(this.rotateSpeed / 0.68, 0.14, 1.0);
    const radiansPerPixel = (0.00018 + 0.00078 * zoomFactor) * speedFactor;
    const dTheta = -dx * radiansPerPixel;
    const dPhi = -dy * radiansPerPixel;
    const instantTheta = clamp(dTheta * (16.6667 / dt), -0.012, 0.012);
    const instantPhi = clamp(dPhi * (16.6667 / dt), -0.009, 0.009);

    state.spherical.theta += dTheta;
    state.spherical.phi += dPhi;
    state.velocityTheta = state.velocityTheta * 0.78 + instantTheta * 0.22;
    state.velocityPhi = state.velocityPhi * 0.78 + instantPhi * 0.22;
  ...
  state.onPointerUp = (event) => {
    ...
    state.velocityTheta *= 0.82;
    state.velocityPhi *= 0.82;
    if (Math.abs(state.velocityTheta) + Math.abs(state.velocityPhi) > 0.000009) startInertia(this, state);
    event.preventDefault();
  };
  ...
  state.onWheel = (event) => {
    ...
    const adaptiveZoom = (0.22 + 0.78 * close) * clamp(this.zoomSpeed / 0.72, 0.18, 1.15);
    const nextRadius = clamp(
      oldRadius * Math.exp(event.deltaY * 0.00088 * adaptiveZoom),
      this.minDistance,
      this.maxDistance,
    );
...
const controlsProto = OrbitControls.prototype;
controlsProto.connect = connectHeavy;
controlsProto.disconnect = disconnectHeavy;
controlsProto.dispose = disconnectHeavy;
controlsProto.update = updateHeavy;

function applyCityLod(scene, camera) {
  const distance = camera.position.length();
  const capitalGate = 1.58;
  const regionalGate = 1.24;
  const surfaceRadius = 1.0012;
  ...
  scene.traverse((object) => {
    ...
    if (Math.abs(object.position.length() - surfaceRadius) > 0.00001) {
      object.position.setLength(surfaceRadius);
    }
    ...
    const horizon = Math.min(0.992, 1 / Math.max(distance, 1.001) + 0.004);
...
Object.defineProperty(css2dProto, 'render', {
  ...
  set(originalRender) {
    const wrapped = function tunedCityRender(scene, camera) {
