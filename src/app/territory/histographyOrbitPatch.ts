...
type PatchedControls = OrbitControls & {
...
const states = new WeakMap<OrbitControls, ControllerState>();
const proto = OrbitControls.prototype as unknown as Record<PropertyKey, unknown>;

const clamp = (value: number, min: number, max: number) => Math.min(max, Math.max(min, value));

function dispatchChange(controls: OrbitControls) {
  controls.dispatchEvent({type: 'change'} as never);
}

function syncSpherical(controls: PatchedControls, state: ControllerState) {
...
function applySpherical(controls: PatchedControls, state: ControllerState) {
...
function globeHit(controls: PatchedControls, state: ControllerState, clientX: number, clientY: number) {
...
function stopInertia(state: ControllerState) {
  if (state.raf !== null) cancelAnimationFrame(state.raf);
  state.raf = null;
}

function startInertia(controls: PatchedControls, state: ControllerState) {
  stopInertia(state);
...
function connectPatched(this: PatchedControls) {
  ...
  state.velocityPhi = 0;
  state.raf = null;
  state.previousTouchAction = dom.style.touchAction;
  state.spherical = new THREE.Spherical();
  state.offset = new THREE.Vector3();
  state.raycaster = new THREE.Raycaster();
  state.ndc = new THREE.Vector2();
  state.sphere = new THREE.Sphere(new THREE.Vector3(0, 0, 0), 1.0004);
  ...
  state.onWheel = (event: WheelEvent) => {
    stopInertia(state);
    syncSpherical(this, state);
    ...
    const nextRadius = clamp(oldRadius * Math.exp(event.deltaY * 0.00105 * adaptiveZoom), this.minDistance, this.maxDistance);
...
function disconnectPatched(this: PatchedControls) {
...
function updatePatched(this: PatchedControls) {
  const state = states.get(this);
